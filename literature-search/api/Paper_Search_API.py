...
def run_server(port=8080):
    server = HTTPServer(('0.0.0.0', port), VercelHandler)
    print("=" * 60)
    print("🚀 基因变异文献搜索 API 本地测试服务器")
    print("=" * 60)
    print(f"📍 地址: http://localhost:{port}")
    print("\n📌 测试命令:")
    print("-" * 60)
    print("\n1️⃣  HGVS 格式:")
    print(f'  curl "http://localhost:{port}/api/search?type=hgvs&gene=COL4A3&hgvs=NM_000484.4:c.2059A>C&hgvs_p=p.Leu1598Arg"')
    print("\n2️⃣  VCF 格式:")
    print(f'  curl "http://localhost:{port}/api/search?type=vcf&gene=COL4A3&chrom=7&pos=140453136&ref=A&alt=T"')
    print("\n3️⃣  RSID 格式:")
    print(f'  curl "http://localhost:{port}/api/search?type=rsid&gene=COL4A3&rsid=rs113994087"')
